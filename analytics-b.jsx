...
const RB = window.Recharts;
const { BarChart: BarChartB, Bar: BarB, PieChart: PieChartB, Pie: PieB, Cell: CellB,
  XAxis: XAxisB, YAxis: YAxisB, CartesianGrid: CartGridB, Tooltip: TooltipB,
  ResponsiveContainer: RCB, Legend: LegendB } = RB;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16 }}>
        <div className="card" style={{ padding: '18px 22px' }}>
          ...
          <div style={{ height: 240 }}>
            <RCB>
              <BarChartB data={riskDrivers} layout="vertical" margin={{ top: 4, right: 20, left: 10, bottom: 0 }}>
                <CartGridB strokeDasharray="3 3" stroke="#F3F4F6" horizontal={false} />
                <XAxisB type="number" stroke={COLORS.textMuted} fontSize={11} tickLine={false} axisLine={false} tickFormatter={v => v + '%'} />
                <YAxisB type="category" dataKey="driver" width={120} stroke={COLORS.textPrimary} fontSize={12} tickLine={false} axisLine={false} />
                <TooltipB formatter={(v) => `${v}% impact`} />
                <BarB dataKey="impact" fill={COLORS.chartCoral} radius={[0, 6, 6, 0]} />
              </BarChartB>
            </RCB>
          </div>
        </div>
        <div className="card" style={{ padding: '18px 22px' }}>
          ...
          <div style={{ height: 240, position: 'relative' }}>
            <RCB>
              <PieChartB>
                <PieB data={distribution} dataKey="value" innerRadius={50} outerRadius={85} paddingAngle={2} stroke="none">
                  {distribution.map((d, i) => <CellB key={i} fill={d.color} />)}
                </PieB>
                <TooltipB formatter={(v) => `${v}%`} />
              </PieChartB>
            </RCB>
...
function BenchmarkingTab({ survey }) {
  const [sub, setSub] = React.useState('industry');
  const subs = [
...
function QuestionTab({ survey }) {
  ...
  React.useEffect(() => { setIdx(0); }, [survey.key]);
        ...
        <div className="card" style={{ padding: '18px 22px' }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <div style={{ width: 180, height: 180, position: 'relative' }}>
              <RCB>
                <PieChartB>
                  <PieB data={q.donut.map(d => ({ name: d.name, value: d.v }))} dataKey="value" innerRadius={55} outerRadius={80} paddingAngle={2} stroke="none">
                    {q.donut.map((d, i) => <CellB key={i} fill={d.c} />)}
                  </PieB>
                  <TooltipB formatter={(v) => `${v}%`} />
                </PieChartB>
              </RCB>
...
function ParticipationTab({ survey }) {
  ...
  const [channels, setChannels] = React.useState({ email: true, whatsapp: true, inapp: false });
  const [nudgeTargets, setNudgeTargets] = React.useState(['APAC CS', 'Sales Americas']);
  const [nudgeMsg, setNudgeMsg] = React.useState('Hi team — we have 4 days left to complete the ' + survey.short + '. Your feedback directly shapes what we act on. Takes ~5 minutes.');
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 16 }}>
        <div className="card" style={{ padding: '18px 22px' }}>
          ...
          <div style={{ height: 220, position: 'relative' }}>
            <RCB>
              <PieChartB>
                <PieB data={donut} dataKey="value" innerRadius={60} outerRadius={88} paddingAngle={2} stroke="none">
                  {donut.map((d, i) => <CellB key={i} fill={d.color} />)}
                </PieB>
                <TooltipB formatter={(v) => `${v}%`} />
              </PieChartB>
            </RCB>
